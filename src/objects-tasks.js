...
function mergeObjects(objects) {
  ...
  objects.forEach((item) => {
    Object.entries(item).forEach(([key, value]) => {
      resultObject[key] = resultObject[key] ? resultObject[key] + value : value;
    });
  });
  return resultObject;
}
...
function removeProperties(obj, keys) {
  const copy = { ...obj };
  if (typeof keys === 'string') return delete copy[keys];
  keys.forEach((item) => delete copy[item]);
  return copy;
}
...
function sellTickets(queue) {
  ...
  for (let i = 1; i < queue.length; i += 1) {
    if (queue[i] - 25 > sum) return false;
    sum += 25;
  }
  return true;
}
...
const cssSelectorBuilder = {
  ...
  combine(elector1, combinator, selector2) {
    const newSelector = { ...this };
    newSelector.selector = `${elector1.selector} ${combinator} ${selector2.selector}`;
    return newSelector;
  },
